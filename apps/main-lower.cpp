...
int main(int argc, char **argv)
{
	...
	auto reorderdd = dd->dynamicReorder(functionality, varMap, dd::DynamicReorderingStrategy::lbLinearSifting);
	// end=clock();
	...
	for(int i=0; i<100; ++i)
	{
		reorderdd = dd->dynamicReorder(reorderdd, varMap, dd::DynamicReorderingStrategy::lbLinearSifting);
	...
	cout<< "lower???:" <<fileName<<' '<<dd_size<<' '<<one_sifting_size<<' '<<one_sifting_time<<' '<<conv_sifting_size<<' '<<conv_sifting_time<<endl;
